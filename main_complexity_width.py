...
args = get_args()
experiment = Experiment(api_key='98Hhyb58cThYVpxaOvbL3Yu8S', project_name='trhic-complexity-corruption2', workspace='valthom')
...
while t <= 150000:
    for data, labels in train_loader:
        ...
        if t % log_step == 0:
            ...
            fr_tr = compute_fisher_rao_norm(args, train_loader, model)
            fr_te = compute_fisher_rao_norm(args, test_loader, model)
            l2_norm = compute_L2_norm(args, model)
            #writer.add_scalar( t)
            #writer.add_scalar( t)
        ...
        if t % log_step  == 0:
            ...
            #
                ...
                for k in [1, 5, 10, 30, 100, 200, 300, 500, 1000]:
                    if k <= n_param:
