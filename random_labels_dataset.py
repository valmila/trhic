...
class corruptCIFAR10(datasets.CIFAR10):
  ...
  def __init__(self, corrupt_prob=0.0, num_classes=10, **kwargs):
    super(corruptCIFAR10, self).__init__(**kwargs)
...
class corruptSVHN(datasets.SVHN):
  """SVHN dataset, with support for randomly corrupt labels.
  Params
  ------
  corrupt_prob: float
    Default 0.0. The probability of a label being replaced with
  ...
  """
  def __init__(self, corrupt_prob=0.0, num_classes=10, **kwargs):
    super(corruptSVHN, self).__init__(**kwargs)
    self.n_classes = num_classes
    if corrupt_prob > 0:
      self.corrupt_labels(corrupt_prob)
...
class corruptMNIST(datasets.MNIST):
  ...
  """
  def __init__(self, corrupt_prob=0.0, num_classes=10, **kwargs):
    super(corruptMNIST, self).__init__(**kwargs)
    self.n_classes = num_classes
    if corrupt_prob > 0:
      self.corrupt_labels(corrupt_prob)
